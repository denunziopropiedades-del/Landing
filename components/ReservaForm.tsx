...
export default function ReservaForm({
  ...
  const onSubmit = async (data: ReservaInput) => {
    ...
    try {
      ...
      const mensaje = `Hola Matías. Quiero confirmar mi reserva del ${lote?.nombre ?? "lote"} — Manzana ${data.manzana}. Nombre: ${data.nombre} ${data.apellido}, DNI: ${data.dni}, Tel: ${data.telefono}.`;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-5 rounded-2xl border border-black/5 bg-white p-8 shadow-sm sm:grid-cols-2">
      ...
      <div>
        <label className="mb-1.5 block text-sm font-medium text-brand-black/80">Manzana</label>
        <select {...register("manzana")} defaultValue="" className="w-full rounded-lg border border-black/10 px-4 py-2.5 focus:border-brand-green-600 focus:outline-none">
          <option value="" disabled>
            Seleccioná una manzana
          </option>
          {manzanas.map((m) => (
        ...
        {errors.manzana && <p className="mt-1 text-xs text-red-600">{errors.manzana.message}</p>}
      </div>

      <div className="sm:col-span-2">
        <label className="mb-1.5 block text-sm font-medium text-brand-black/80">Lote</label>
        <select {...register("lote")} defaultValue="" className="w-full rounded-lg border border-black/10 px-4 py-2.5 focus:border-brand-green-600 focus:outline-none">
          <option value="" disabled>
            Seleccioná un lote
          </option>
          {lotes.map((l) => (
        ...
        {errors.lote && <p className="mt-1 text-xs text-red-600">{errors.lote.message}</p>}
      </div>
